...
def train(args, model, train_loader, optimizer, epoch, training_step, writer):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if training_step == 1 or training_step == 2:
            ...
            # TEST: reverse input
            model_input_reverse = torch.flip(pred_traj_fake_rel, dims=[0])
            pred_traj_fake_rel_reverse = model(
                model_input_reverse, obs_traj_reverse, seq_start_end, 1, training_step
            )  # second parameter is never used, so no matter what is passed in
            ...
            for _ in range(args.best_k):
                ...
                # TEST reverse input
                model_input_reverse = torch.cat(
                    (torch.flip(pred_traj_fake_rel, dims=[0]), obs_traj_rel_reverse),
                    dim=0,
                )
...
def validate(args, model, val_loader, epoch, writer):
    ...
    with torch.no_grad():
        for i, batch in enumerate(val_loader):
            ...
            # TEST: reverse input
            pred_traj_fake_rel_predpart_reverse = pred_traj_fake_rel_reverse[
                -(20 - args.pred_len) :
            ]
            pred_traj_fake_reverse = relative_to_abs(
                pred_traj_fake_rel_predpart_reverse, obs_traj_reverse[-1]
            )
            ...
            # TEST: reverse input
            ade__reverse, fde__reverse = cal_ade_fde(
                obs_traj_reverse, pred_traj_fake_reverse
            )
            ade__reverse = ade__reverse / (
                obs_traj_reverse.shape[1] * (20 - args.pred_len)
            )
            fde__reverse = fde__reverse / (obs_traj_reverse.shape[1])

            ade.update(
                ade_ + ade__reverse, obs_traj.shape[1] + obs_traj_reverse.shape[1]
            )
            fde.update(
                fde_ + fde__reverse, obs_traj.shape[1] + obs_traj_reverse.shape[1]
            )
